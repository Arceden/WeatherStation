var Listener = function () {
  ...
  this.newElements = function (name, elements, PopupHandler) {
    ...
    if (this.list[name] == undefined) this.list[name] = JSON.parse(JSON.stringify(this.default_item)); //If a handler has been provided, add it to the list

    if (PopupHandler != null) this.list[name].popupHandler = PopupHandler;
    if (elements == null) console.warn("Could not find any elements for " + name); //Add all elements to the list
  ...
  this.applyHandler = function (name) {
    var item = this.list[name]; // Loop through all elements to open
